...
* Program Layout
  ...
  * You can think of a program statement as a series of tokens[fn:9]:
    #+name: tokenization
    #+begin_example
     printf ( "Height: %d\n"   ,   height )  ;
       1    2        3         4     5    6  7
  ...
  * You can have any amount of white (empty) space between program
    tokens (this is not so for all programming languages[fn:10]).
...
[fn:10]Python e.g. is white-space sensitive: the indentation level is
significant, it denotes code blocks, and needs to be consistent. The
same goes for Org-mode markdown and code blocks.

[fn:9]The tokenization is an important sub-process of natural language
