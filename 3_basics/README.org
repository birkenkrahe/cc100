...
* Putting it all together (C program)
  ...
  * [X] Now for the final program [[dweight_c]]. This time, we allow for
    ...
    #+name: dweight_c
    #+begin_src C :tangle ./src/dweight.c :results raw
      #include <stdio.h>
...
* Reading input
  ...
  * [ ] Modify the program [[iscan]] to read a ~float~ instead of an
    ~int~.

  * [ ] Let's modify the dimensional weight program so that the
    ~height~, ~length~ and ~width~ of the box must be entered.

  * If you don't have a copy on your computer, get it from GitHub (in
    ~cc100/3_basics/src/dweight.c~
