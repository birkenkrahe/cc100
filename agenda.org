...
** Understand tangling C code from Org-mode
   ...
   * Review: Can you answer the following questions?[fn:3]
     | What is Emacs?                             |
     | What is Org-mode?                          |
...
** PRACTICE - dimensional weight reconsidered
   ...
   * [ ] Note that the number ~166~ is now defined as ~const~.
   ...
   * [ ] Copy the code block into a second code block, and name it
     ~dweight1~.
   ...
   * [ ] Your code block should now look like this (without the comments):
     #+name: dweight1
     #+begin_src C 
       #include <stdio.h>
   ...
   * [ ] Tangle the code block with the key sequence ~C-c C-v t~ to
     get the C source code file ~dweight1.c~

   * [ ] Compile and run it on the Windows command line, and you're done!
     #+caption: sample result on the Windows CMD line
     #+attr_html: :width 600px
     [[./img/dweight.png]]

   * [ ] Alternatively, you can pass the input data to the file using
