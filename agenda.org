...
* Lab session: reading input - w7s18 (02/25/22)
** Objectives

   * [ ] Review: compile and run on the commandline (CLI)
   * [ ] Review: quiz questions
   * [ ] Understand tangling code once and for all
   * [ ] Alter program to take input from the CLI
   * [ ] Complete program from source to finish [PDF printout]

** Be the compiler! (From Quiz 5)

   The following program compiles and runs but it does not print the
   right results - it always prints ~1~, no matter what is
   entered. What is the problem?
   ...
   Explanation: if you look at the ~scanf~ manual page, you read that
   it "returns the number of input items successfully matched and
   assigned." For ~val=scanf("%d",&input);~ only ~1~ ~int~ was
   scanned, not the integer value of the scanned character (~input~).

   Examples:
   #+begin_src C :results output :tangle scanf.c
     int x, y, z, u, n;
     n = scanf("%d", &x);
     printf("n = %d\n", n);                 // prints 1
     n = scanf("%d%d", &x, &y);
...
** PRACTICE - dimensional weight reconsidered
   ...
   1. [ ] If your neighbor does not get along, help him/her without
      being asked explicitly!

   2. [ ] Checklist before you start coding:
   ...
   3. [ ] Create an Org-mode file "dweight.org"

   4. [ ] Add one named C code block (use <s [TAB] to do this)

   5. [ ] Name the code block ~dweight~

   6. [ ] Start with the code that we created last week, or take the
   ...
   7. [ ] Check: your code block should now look like this:
      #+name: dweight
      #+begin_src C
        #include <stdio.h>
   ...
   8. [ ] You do not need any code block header arguments except ~C~
      and ~:results output~.

   9. [ ] Note that the number ~166~ is now defined as ~const~.

   10. [ ] Run the code block in Emacs with ~C-c C-c~ to make sure it
   ...
   11. [ ] Copy the code block into a second code block, and name it
       ~dweight1~.

   12. [ ] Alter the code so that the three variables ~height~,
   ...
   13. [ ] Your code block should now look like this (without the comments):
       #+name: dweight1
       #+begin_src C
         #include <stdio.h>
   ...
   14. [ ] Add the necessary tangle information to the code block header:
       #+begin_example
       :tangle dweight1.c
       #+end_example

   15. [ ] Tangle the code block with the key sequence ~C-c C-v t~ to
       get the C source code file ~dweight1.c~

   16. [ ] Compile and run it on the Windows command line, and you're done!
       #+caption: sample result on the Windows CMD line
       #+attr_html: :width 600px
       [[./img/dweight.png]]

   17. [ ] Alternatively, you can pass the input data to the file using
       the redirection operator ~<~ - then you can run the file inside
       Emacs (either on the shell, or using the code block header). You
       can find sample solutions for this here in GitHub (dweight_emacs.org).
