...
** PRACTICE - dimensional weight reconsidered
   ...
   * [ ] Your code block should now look like this (without the comments):
     #+name: dweight1
     #+begin_src C :results output :cmdline < dweight_input :tangle dweight1.c
       #include <stdio.h>
     ...
     #+RESULTS: dweight1
     : Enter the box height: Enter the box length: Enter the box width: Dimensions: 10x12x8
     : Volume (cubic inches): 960
     : Dimensional weight (pounds): 6
   ...
   * [ ] Tangle the file with the key sequence ~C-c C-v t~.

   * [ ] Compile and run it on the Windows command line:
     #+begin_src bash
       gcc -o dweight dweight

     #+end_src

     , or with redirection using an input file
