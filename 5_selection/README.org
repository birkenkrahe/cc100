...
* The switch structure
  ...
  * The ~switch~ statement is fairly complex: it combines conditional
  ...
  1. *Controlling expression:* an integer expression in
     parentheses. Characters are treated as integers in C and cannot be
     tested.

  2. *Case labels:* each case begins with a constant expression - this
     is like any other expression except that it cannot contain
